...
from utils import replace_oovs,source2ids,outputids2words,Beam,add2heap
...
class Predict():
    ...
    def beam_search(self,
                    ...
                    x_padding_masks):
        x_copy = replace_oovs(x, self.vocab)
        encoder_output, encoder_states = self.model.encoder(x_copy)
        ###########################################
        coverage_vector = torch.zeros((1, x.shape[1])).to(self.device)
    ...
    def best_k(self, beam, k, encoder_output, x_padding_masks, x, len_oovs):
        ...
        # print(log_probs[self.vocab.EOS])
        # print(x.size()[1])
    ...
    def predict(self, text, beam_search=False):
